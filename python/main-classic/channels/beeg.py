...
def videos(item):
    ...
    # Lista de IDs
    id_list = []
    id_string = scrapertools.get_match(data,"var tumb_id  =\[([^\]]+)\]")
    ...
    # Lista de titulos
    title_list = []
    title_string = scrapertools.get_match(data,"var tumb_alt \=\[([^\]]+)\]")
...
def listcategorias(item):
    ...
    data = scrapertools.cache_page(item.url)
    data = scrapertools.get_match(data,'<!-- TAGS -->(.*?)<!-- /TAGS -->')
    patron = '<li><a target="_self" href="([^"]+)"[^>]+>([^"]+)</a></li>'
    matches = re.compile(patron,re.DOTALL).findall(data)
    itemlist = []
    itemlist.append( Item(channel=__channel__, action="videos" , title="Long videos", url="http://beeg.com/tag/long+videos/" ))
